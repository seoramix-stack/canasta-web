...
class CanastaGame {
    ...
    meldMultiple(playerIndex, groups) {
        if (playerIndex !== this.currentPlayer || this.turnPhase !== "playing") 
            return { success: false, message: "Not your turn!" };

        let hand = [...this.players[playerIndex]]; // Copy for validation
        let teamMelds = (playerIndex % 2 === 0) ? this.team1Melds : this.team2Melds;
        let totalOpeningPoints = 0;
        let usedIndices = new Set();
        let validationError = null;

        // 1. PRE-VALIDATION SCAN
        groups.forEach(grp => {
            if (validationError) return;

            let rank = grp.rank;
            let indices = grp.indices;
            let cards = indices.map(i => this.players[playerIndex][i]); // Get distinct cards

            // Check Indices duplication
            indices.forEach(idx => {
                if (usedIndices.has(idx)) validationError = "Card used twice!";
                usedIndices.add(idx);
            });

            // Check Rank Purity (No Wilds in Multi-Meld for simplicity)
            if (cards.some(c => c.rank !== rank && !c.isWild)) validationError = `Mixed ranks in ${rank} group.`;
            
            // Check Meld Validity
            let existingMeld = teamMelds[rank];
            if (!existingMeld && cards.length < 3) validationError = `New ${rank} meld needs 3+ cards.`;

            // Calculate Points
            totalOpeningPoints += cards.reduce((sum, c) => sum + this.getCardValue(c.rank), 0);
        });

        if (validationError) return { success: false, message: validationError };

        // 2. OPENING REQUIREMENT CHECK (Aggregate)
        let isOpening = (Object.keys(teamMelds).length === 0);
        if (isOpening) {
            let teamScore = (playerIndex % 2 === 0) ? this.cumulativeScores.team1 : this.cumulativeScores.team2;
            let req = this.getOpeningReq(teamScore);
            if (totalOpeningPoints < req) {
                return { success: false, message: `Total ${totalOpeningPoints} < Req ${req} pts.` };
            }
        }

        // 3. CANASTA OUT CHECK
        // Predict future state to see if they can go out
        let cardsRemaining = hand.length - usedIndices.size;
        let currentCanastas = Object.values(teamMelds).filter(p => p.length >= 7).length;
        
        // Add predicted new canastas
        groups.forEach(grp => {
            let existingLen = teamMelds[grp.rank] ? teamMelds[grp.rank].length : 0;
            if (existingLen < 7 && (existingLen + grp.indices.length) >= 7) {
                currentCanastas++;
            }
        });

        if (cardsRemaining === 0) { // Going out floating?
             if (currentCanastas < this.config.MIN_CANASTAS_OUT) {
                 return { success: false, message: `Need ${this.config.MIN_CANASTAS_OUT} Canastas to float.` };
             }
        } else if (cardsRemaining === 1) { // Going out with discard?
             // Not technically going out *yet* (need to discard), but check if they blocked themselves
             // Actually, standard play allows melding down to 1 card even if you can't go out, 
             // providing you keep holding that card. So no error here.
        }

        // 4. EXECUTION (Atomic Removal)
        // Sort ALL indices descending to remove safely
        let allIndices = Array.from(usedIndices).sort((a, b) => b - a);
        
        // Extract Cards Object before deleting from array
        let cardsToMeld = [];
        groups.forEach(grp => {
            let cards = grp.indices.map(i => this.players[playerIndex][i]);
            cardsToMeld.push({ rank: grp.rank, cards: cards });
        });

        // Remove from Hand
        allIndices.forEach(idx => this.players[playerIndex].splice(idx, 1));

        // Add to Table
        cardsToMeld.forEach(item => {
            if (!teamMelds[item.rank]) teamMelds[item.rank] = [];
            teamMelds[item.rank].push(...item.cards);
        });
        ...
        if (this.players[playerIndex].length === 0) {
            this.turnPhase = "game_over";
            this.finalScores = this.calculateScores(playerIndex); 
            console.log(`ROUND OVER: Player ${playerIndex} went out (Multi-Meld Float).`);
            return { success: true, message: "GAME_OVER" };
        }
